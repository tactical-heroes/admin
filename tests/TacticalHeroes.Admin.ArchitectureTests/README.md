...
унаследованная логика проверяется в тестах объявляющего её базового класса.

Если один поведенческий сценарий также проверяет другой метод, его можно
связать с ним стандартным `[Trait("Covers", "MethodName")]`. Вызовы закрытых
методов проверяются через UI или доступное поведение, без reflection-вызовов.
Один тест не может дважды засчитаться для перегрузок одного имени.
...
Архитектурный проект автоматически ссылается на production и ComponentTests
проекты через MSBuild glob, поэтому новый компонент или модуль попадает под правило.
